...
class Scheduler:
    ...
    def start(self) -> None:
        ...
        for ether in WEEKEND_ETHERS:
            self._scheduler.add_job(self.start_ether, 'cron', day_of_week="sat-sat", hour=ether['start'].hour, minute=ether['start'].minute, args=(self._bot, self._async_sessionmaker, ether['start']))
            self._scheduler.add_job(self.start_ether, 'cron', day_of_week="sun", hour=ether['end'].hour, minute=ether['end'].minute, args=(self._bot, self._async_sessionmaker, ether['end']))
